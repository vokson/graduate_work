...
## Deploy
Сервис запущен и работает по адресу http://51.250.10.146/. Пароль везде "qwe12345".
...
## Демо
Краткое опишу интерфейс пользователя, чтобы было понятно куда нажимать. В случае каких-либо глюков смело обновляем страницу.

### Вход
1. Войти и получить токены
2. Выйти и сбросить токены

![entrance](docs/login.png)

### Главная страница
1. Кнопка перехода
1. Информация о пользователе и разрешения
1. Контейнер для перетаскивания файлов и кнопка загрузки (если Drag&Drop в браузере не работает по каки-либо причинам)
1. История действий пользователя
1. Общедоступные ссылки на файл (обновляются при выборе файла)
1. Строки с информацией о файлах пользователя
1. Информация о серверах и checkbox активации
1. Checkbox выбора файла
1. Иконка, показывающая размещение файла на сервере
1. Кнопка удаления файла
1. Кнопка создания общедоступной ссылки на файл

![main page](docs/main_page.png)

1. Нужно выбрать файл, чтобы появилось окно переименования
1. Кнопка смены имени файла

![rename](docs/rename.png)

### CDN
Список серверов. При выборе строки слева появляется детальная информация

![rename](docs/servers.png)

Детальная информация о сервере. Кнопки Обновления и Удаления

![rename](docs/server_details.png)
