...
async def enrich_cdn_server_by_files(
    ...
    async with uow:
        ...
        if new_server.is_on and new_server.is_active:
            servers_in_zone = await uow.cdn_servers.get_switched_on(True, new_server.zone)
            servers_ids_in_zone = [x.id for x in servers_in_zone]

            files_on_servers = {}
            for server_id in servers_ids_in_zone:
                if server_id != new_server.id:
                    files_on_servers[server_id] = await uow.files.get_non_deleted_on_servers([server_id])

            files_on_new_server = await uow.files.get_non_deleted_on_servers([new_server.id])
            ...
            for server_id in files_on_servers:
                for file in files_on_servers[server_id]:
                    if file.id not in files_ids_on_new_server:
                        logger.info(f'Order file {file.id} to download from CDN server {server_id}')
                        uow.push_message(commands.OrderFileToDownload(
                            file_id=file.id,
                            server_id=server_id
                        ))
...
async def get_upload_link(
    ...
    async with uow:
        ...
        obj = await uow.files.get_non_deleted_by_name_and_user_id(cmd.name, cmd.user_id)
...
async def handle_s3_event(
    ...
    logger.info(f"Handling S3 event with key {cmd.routing_key}, body {cmd.body}")
    async with uow:
        try:
...
async def handle_service_event(
    ...
    logger.info(f"Handling service event with key {cmd.routing_key}, body {cmd.body}")
...
async def mark_file_as_stored(
    ...
    async with uow:
        ...
        ids_of_servers_where_file_located = await uow.files.get_ids_of_servers(cmd.file_id)
        ids_of_servers_in_zone = [x.id for x in await uow.cdn_servers.get_switched_on(True, server.zone)]
    ...
    if len(set(ids_of_servers_in_zone) - set(ids_of_servers_where_file_located)) == 0:
...
async def download_file_to_temp_storage(
    ...
    logger.info(f"Downloading file with id {cmd.file_id} from server with id {cmd.server_id}")
    async with uow:
        server = await uow.cdn_servers.get_by_id(cmd.server_id)
    ...
    uow.push_message(events.FileDownloadedToTempStorage(id=cmd.file_id, zone=server.zone))
    logger.info(f"File with id {cmd.file_id} has been downloaded to temp storage")
    return command_results.PositiveCommandResult({})
...
async def order_file_to_copy(
    ...
    logger.info(f"Ordering file with id {cmd.file_id} to copy to server with id {cmd.server_id}")
...
async def copy_file(
    ...
    logger.info(f"Coping file with id {cmd.file_id} to copy to server with id {cmd.server_id}")
    async with uow:
        server = await uow.cdn_servers.get_by_id(cmd.server_id)
        storage = await uow.s3_pool.get(server.name, server.host, server.port)
        filename = str(cmd.file_id)
    ...
    logger.info(f"File with id {cmd.file_id} has been copied to server with id {cmd.server_id}")
    return command_results.PositiveCommandResult({})
...
async def remove_file_from_temp_storage(
    ...
    try:
        ...
        os.remove(path)
        logger.info(f"File with id {cmd.file_id} has been removed temp storage")

    except FileNotFoundError:
        logger.warning(f"File with id {cmd.file_id} was not found in temp storage")
        pass
...
async def order_file_to_remove(
    ...
    logger.info(f"Ordering file with id {cmd.file_id} to remove from server with id {cmd.server_id}")
...
async def remove_file(
    ...
    logger.info(f"Removing file with id {cmd.file_id} from server with id {cmd.server_id}")
    async with uow:
        server = await uow.cdn_servers.get_by_id(cmd.server_id)
    ...
    logger.info(f"File with id {cmd.file_id} has been removed from server with id {cmd.server_id}")
    return command_results.PositiveCommandResult({})
...
async def mark_file_as_removed(
    ...
    logger.info(f"Marking file with id {cmd.file_id} as removed from server with id {cmd.server_id}")
    async with uow:
...
async def get_file_share_link(
    ...
    logger.info(f"Getting single share links for file with id {cmd.file_id}")
    async with uow:
        file = await uow.files.get_by_id(cmd.file_id)
        if not file:
            raise exceptions.FileDoesNotExist
    ...
    logger.info(f"Info about share link {cmd.link_id} for file with id {cmd.file_id} has been returned")
    return command_results.PositiveCommandResult(
...
async def delete_file_share_link(
    ...
    logger.info(f"Deleting share link {cmd.link_id} for file with id {cmd.file_id}")
    async with uow:
        file = await uow.files.get_by_id(cmd.file_id)
        if not file or file.user_id != cmd.user_id:
            raise exceptions.FileDoesNotExist
    ...
    logger.info(f"Share link {cmd.link_id} for file with id {cmd.file_id} has been deleted")
    return command_results.PositiveCommandResult({})
...
async def validate_file_share_link(
    ...
    logger.info(f"Validating share link {cmd.link_id} for file with id {cmd.file_id}")
    async with uow:
        file = await uow.files.get_by_id(cmd.file_id)
        if not file:
            raise exceptions.FileDoesNotExist
    ...
    logger.info(f"Share link {cmd.link_id} for file with id {cmd.file_id} has been validated")
    return command_results.PositiveCommandResult({})
